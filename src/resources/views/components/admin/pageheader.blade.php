<div {{ $attributes->merge(['class' => 'bg-firefly-50 dark:bg-slate-900 dark:text-firefly-300 flex items-center justify-between my-5 p-14 shadow rounded-lg']) }}>
    ...
    <div class="hidden md:block">
        <span class="flex items-center gap-1.5 py-1.5 px-3 rounded-full text-xs font-medium
         bg-firefly-200 text-gray-800 dark:bg-black dark:text-white uppercase">
            {{ $slot ?? ' ' }}
            {{ $modelName ?? ' ' }}
        </span>
    </div>
